...
namespace VixenPlus
{
    internal class Engine8 : IDisposable, IQueryable
    {
        ...
        {
            get {
                return _secondaryEngine != null ? _secondaryEngine.IsRunning : _isRunning;
            }
        }
        ...
        public void Dispose()
        {
            ...
            if (_plugInRouter != null)
            {
                ...
                {
                    MessageBox.Show(Resources.engineShutDownError + exception.Message, Vendor.ProductName,
        ...
        private void InitEngineContext(ref EngineContext context, int sequenceIndex)
        {
            ...
            else
            {
                ...
                if (context.RouterContext == null)
                {
                    context.RouterContext = _plugInRouter.CreateContext(new byte[executableObject.ChannelCount],
                                                                        _useSequencePluginData
                                                                            ...
                                                                            : CurrentObject.SetupData, executableObject,
                                                                        _surfacedTimer);
                }
                ...
                context.FadeStartTickCount = ((CurrentObject.CrossFadeLength == 0) ||
                                              (CurrentObject.EventSequences.Count == 1))
                                                 ? 0
                                                 : ((IsLooping || (CurrentObject.EventSequences.Count > (sequenceIndex + 1)))
                                                        ? (executableObject.Time - (CurrentObject.CrossFadeLength*1000))
                                                        : 0);
        ...
        {
            var builder = new StringBuilder("         Name: " + mappedPlugIn.PlugIn.Name) 
                .AppendLine("         From: " + mappedPlugIn.From) 
                .AppendLine("         To: " + mappedPlugIn.To) 
                .AppendLine("         Enabled: " + mappedPlugIn.Enabled);
            return builder.ToString();
        }
        ...
        private void TimerTick(EngineContext context)
        {
            ...
            context.TickCount = (context.SoundChannel != null) && context.SoundChannel.IsPlaying
                                    ? (int) context.SoundChannel.Position 
                                    : context.StartOffset + ((int) context.Timekeeper.ElapsedMilliseconds);
