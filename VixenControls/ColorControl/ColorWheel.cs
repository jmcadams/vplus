...
namespace CommonControls {
    ...
    public sealed class ColorWheel : Control, IColorEditor {
        ...
        protected override void OnKeyDown(KeyEventArgs e) {
            ...
            if (HslColor.IsNearlyEqual(hue, color.H)) {
                color.H = hue;
        ...
        private void CalculateWheel() {
            ...
            if (ClientSize.Width > 16 && ClientSize.Height > 16) {
                ...
                for (double angle = 0; angle < 360; angle += ColorStep) {
                    ...
                    var location = GetColorLocation(angleR, radius);
        ...
        private PointF GetColorLocation(HslColor color) {
            var angleR = color.H * Math.PI / 180;
            double radius = GetRadius(_centerPoint);
            radius *= color.S;
        ...
        private PointF GetColorLocation(double angleR, double radius) {
            var x = Padding.Left + _centerPoint.X + Math.Cos(angleR) * radius;
            var y = Padding.Top + _centerPoint.Y - Math.Sin(angleR) * radius;

            return new PointF((float)x, (float)y);
        }


        private float GetRadius(PointF centerPoint) {
        ...
        private void OnSelectionSizeChanged(EventArgs e) {
            ...
            var handler = SelectionSizeChanged;
        ...
        private void PaintCurrentColor(PaintEventArgs e) {
            if (Color.IsEmpty) {
                return;
            }

            var location = GetColorLocation(HslColor);
            ...
            var x = (int)location.X - (SelectionSize / 2);
            var y = (int)location.Y - (SelectionSize / 2);
