namespace Prop1SeqGen
{
    ...
    {
        private const string FILL_FILE = "prop-1_sequencer_fill.txt";
        private const int HIGH_BIT = 0x80;
        private int m_threshold = 0;
        private const string TEMPLATE_FILE = "prop-1_sequencer_main.txt";
        ...
        private List<EventTableRecord> BuildEventTable(EventSequence sequence)
        {
            ...
            for (int i = 0; i < totalEventPeriods; i++)
            {
                ...
                {
                    num = (byte) (num >> 1);
                    num = (byte) (num | ((sequence.EventValues[j, i] >= this.m_threshold) ? 0x80 : 0));
                }
                if ((item != null) && (num == item.Value))
                {
                    item.EventPeriodCount++;
                }
                else
                {
        ...
        {
            string path = Path.Combine(Paths.AddinPath, "prop-1_sequencer_main.txt");
            if (!File.Exists(path))
            {
                MessageBox.Show("Template file is missing.", Vendor.ProductName, MessageBoxButtons.OK, MessageBoxIcon.Exclamation);
                return false;
            }
            string str2 = Path.Combine(Paths.AddinPath, "prop-1_sequencer_fill.txt");
            if (!File.Exists(str2))
            {
            ...
            if (addin.ShowDialog() == DialogResult.OK)
            {
                ...
                Label_02FD:
                    ...
                    local1.Value = (byte) (local1.Value | 0x80);
                    if (num3 < list2.Count)
                    {
        ...
        public string Description
        {
            get
            {
                return "Creates a sequence for an EFX-TEK Prop-1 controller.";
            }
        }
        ...
        public string Name
        {
            get
            {
                return "EFX-TEK Prop-1 Sequence Generator";
            }
        }
    }
}
