    ...
    public class _System
    {
        ...
        public RESULT createStream(string name_or_data, MODE mode, ref Sound sound)
        {
            ...
            try
            {
                if (VERSION.platform == Platform.X64)
                {
                    oK = FMOD_System_CreateStream_64(this.systemraw, name_or_data, mode, 0, ref ptr);
                }
                else
                {
                    oK = FMOD_System_CreateStream_32(this.systemraw, name_or_data, mode, 0, ref ptr);
                }
            }
            catch
            {
        ...
        public RESULT setOutput(OUTPUTTYPE output)
        {
            if (VERSION.platform == Platform.X64)
            {
                return FMOD_System_SetOutput_64(this.systemraw, output);
            }
            return FMOD_System_SetOutput_32(this.systemraw, output);
        }
