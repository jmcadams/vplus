namespace RemoteClient
{
    ...
    internal class ControlClient
    {
        ...
        public bool Authenticate(string password)
        {
            ...
            byte socketByte = RemoteClient.Sockets.GetSocketByte(serverConnection.Client);
        ...
        public RemoteClient.ClientStatus ClientStatus(int clientID)
        {
            if (clientID == 0)
            {
                throw new Exception("Invalid client ID");
            ...
            BitConverter.GetBytes(clientID).CopyTo(array, 1);
            serverConnection.Client.Send(array);
            RemoteClient.ClientStatus status = null;
            Socket client = serverConnection.Client;
            if (RemoteClient.Sockets.GetSocketByte(client) == 0x13)
            {
                status = new RemoteClient.ClientStatus();
                status.ExecutionStatus = (ExecutionStatus) RemoteClient.Sockets.GetSocketByte(client);
                if (status.ExecutionStatus != ExecutionStatus.None)
                {
                    status.ProgramName = RemoteClient.Sockets.GetSocketString(client);
                    status.ProgramLength = RemoteClient.Sockets.GetSocketInt32(client);
                    status.SequenceName = RemoteClient.Sockets.GetSocketString(client);
                    status.SequenceLength = RemoteClient.Sockets.GetSocketInt32(client);
                    status.SequenceProgress = RemoteClient.Sockets.GetSocketInt32(client);
                }
            }
            serverConnection.Close();
            return status;
        ...
        {
            TcpClient client = RemoteClient.Sockets.ConnectTo(this.m_serverAddress, 0xa1b9);
        ...
        public VixenExecutionClientStub[] RequestClientEnumeration()
        {
            ...
            Socket client = serverConnection.Client;
            int num = RemoteClient.Sockets.GetSocketInt32(client);
            VixenExecutionClientStub[] stubArray = new VixenExecutionClientStub[num];
            for (int i = 0; i < num; i++)
            {
                stubArray[i] = new VixenExecutionClientStub(RemoteClient.Sockets.GetSocketInt32(client), RemoteClient.Sockets.GetSocketInt32(client), RemoteClient.Sockets.GetSocketString(client));
            }
            serverConnection.Close();
            return stubArray;
        }
        ...
        public string Server
        {
            ...
            set
            {
                if (value.Length != 0)
                {
                    ...
                    {
                        this.m_serverAddress = RemoteClient.Sockets.GetIPV4AddressFor(value);
