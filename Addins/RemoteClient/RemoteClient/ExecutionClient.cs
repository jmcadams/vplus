namespace RemoteClient
{
    ...
    internal class ExecutionClient
    {
        ...
        {
            if (this.m_clientContext.ExecutionInterface.ExecuteChannelToggle(this.m_clientContext.ExecutionContextHandle, RemoteClient.Sockets.GetSocketInt32(socket)))
            {
        ...
        {
            socket.Send(new byte[] { (RemoteClient.Sockets.GetSocketInt32(socket) == this.m_clientNaturalID) ? ((byte) 20) : ((byte) 0x13) });
        }
                    ...
                    if (this.m_listener.Pending())
                    {
                        ...
                        switch (((RequestType) buffer[0]))
                        {
                            ...
                            case RequestType.RetrieveRemote:
                                ClientCommandHandlers.RetrieveRemote(socket, this.m_serverAddress, this.m_clientContext, (RemoteClient.ObjectType) RemoteClient.Sockets.GetSocketByte(socket), RemoteClient.Sockets.GetSocketString(socket));
                                break;
                            ...
                            case RequestType.TimerStatus:
                                this.DoClientTimerStatus(socket, RemoteClient.Sockets.GetSocketByte(socket));
                                break;
        ...
        public bool Register(string clientName)
        {
            ...
            try
            {
                client = RemoteClient.Sockets.ConnectTo(this.m_serverAddress, 0xa1b9);
        ...
        private string[] RequestRemoteList(byte listType)
        {
            ...
            try
            {
                ...
                for (int i = 0; i < num; i++)
                {
                    strArray[i] = RemoteClient.Sockets.GetSocketString(client.Client);
                }
                client.Close();
                return strArray;
            }
        ...
        private void SendExecutionStatus(Socket socket, int executionContextHandle)
        {
            ...
            RemoteClient.Sockets.SendSocketString(socket, str);
            RemoteClient.Sockets.SendSocketInt32(socket, num2);
            ...
            RemoteClient.Sockets.SendSocketString(socket, str);
            RemoteClient.Sockets.SendSocketInt32(socket, num2);
            RemoteClient.Sockets.SendSocketInt32(socket, num);
        }
        ...
        public bool Start()
        {
            if (!this.m_running)
            {
                ...
                try
                {
                    this.m_listener = new TcpListener(RemoteClient.Sockets.GetIPV4Address(), 0xa1bb);
