		...
		private void RangeDialog_FormClosing(object sender, FormClosingEventArgs e) {
			if (this.m_okClick) {
				...
				if ((num < this.m_start) || (num > this.m_end)) {
					MessageBox.Show("Range starting value is invalid", VixenPlus.Vendor.ProductName, MessageBoxButtons.OK, MessageBoxIcon.Hand);
					e.Cancel = true;
				}
				else if ((num2 < this.m_start) || (num2 > this.m_end)) {
					MessageBox.Show("Range ending value is invalid", VixenPlus.Vendor.ProductName, MessageBoxButtons.OK, MessageBoxIcon.Hand);
					e.Cancel = true;
				}
				else if (num > num2) {
					MessageBox.Show("Range starting value cannot be greater than the ending value", VixenPlus.Vendor.ProductName, MessageBoxButtons.OK, MessageBoxIcon.Hand);
					e.Cancel = true;
				}
				else {
					this.m_from = num;
